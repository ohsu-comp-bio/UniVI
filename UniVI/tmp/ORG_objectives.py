...
def m_iwae(model, xs, n_mc_samples=1, beta=1):
    ...
    ''' loss component for modality 1 '''
    lpz1 = pz.log_prob(z1).sum(-1).mean(0)
    lqzx1 = log_mean_exp(torch.stack([qzx1.log_prob(z1).mean(0), qzx2.log_prob(z1).mean(0)])).sum(-1) # oooo
    lpxz1 = log_mean_exp(torch.stack([pxz11.log_prob(x1).sum(-1).mean(0), lik*pxz12.log_prob(x2).sum(-1).mean(0)])) #*********
    print(f'==========================')
    print(f'lpz1: {lpz1.shape}')
    print(f'lqzx1: {lqzx1.shape}')
    print(f'lpxz1: {lpxz1.shape}')
    assert False

    ''' loss component for modality 2 '''
    lpz2 = pz.log_prob(z2).sum(-1).mean(0)
    lqzx2 = log_mean_exp(torch.stack([qzx1.log_prob(z2).mean(0), qzx2.log_prob(z2).mean(0)])).sum(-1) # oooo
    lpxz2 = log_mean_exp(torch.stack([pxz21.log_prob(x1).sum(-1).mean(0), lik*pxz22.log_prob(x2).sum(-1).mean(0)])) #********

    ''' loss component for modality 2 '''
    constraints = ( kld_z12 + kld_z21 )  # xxxxx

    #lw1 = lpxz1 + lpz1 - beta*(lqzx1 + constraints) # test_lpz_term
    #lw2 = lpxz2 + lpz2 - beta*(lqzx2 + constraints) # test_lpz_term

    # Const TEST: beta removed # lw_test
    #lw1 = lpxz1 - (lqzx1 - lpz1 + beta*constraints) # lw_test NEW
    #lw2 = lpxz2 - (lqzx2 - lpz2 + beta*constraints) #  lw_test NEW

    # TEST
    #lw1 = lpxz1 - beta*(lqzx1 - lpz1 + constraints) # 
    #lw2 = lpxz2 - beta*(lqzx2 - lpz2 + constraints) #

    #lw1 = lw1.mean(0) # testj
    #lw2 = lw2.mean(0) # testj

    lw1 = (lpxz1 + lpz1 - 1*lqzx1 - constraints ) #******** retry
    lw2 = (lpxz2 + lpz2 - 1*lqzx2 - constraints ) #******** retry


    lw = log_mean_exp(torch.stack([lw1, lw2]))
    lw = lw.mean(0).sum() ########## oooooooooooooooooooooooo
    #lw = lw.sum() # why???????????? NEW
    ...
    print(f'kld_z12: {kld_z12.sum()/N}')
    print(f'kld_z21: {kld_z21.sum()/N}')
    print(f'constraints: {constraints.sum()/N}')
    print(f'lpz1: {lpz1.sum()/N}')
    print(f'lpz2: {lpz2.sum()/N}')
    print(f'lpxz1: {lpxz1.sum()/N}')
    print(f'lpxz2: {lpxz2.sum()/N}')
    print(f'lqzx1: {lqzx1.sum()/N}')
    print(f'lqzx2: {lqzx2.sum()/N}')
    print(f'lw: {lw/N}')
    #assert False


    #return ( -lw/N, -lw1.mean(0).sum()/N, constraints.sum()/N ) #test_lpz_term
    #return ( -lw, -lw1.mean(0), -lw2.mean(0) )  # lw_test 
    return ( -lw/N, -lw1.sum()/N, -lw2.sum()/N )  # # new # retry
